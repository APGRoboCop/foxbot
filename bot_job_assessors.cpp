...
int assess_JobSeekWaypoint(const bot_t *pBot, const job_struct &r_job) {
   ...
   return jl1[JOB_SEEK_WAYPOINT].basePriority;
}
...
int assess_JobGetUnstuck(const bot_t *pBot, const job_struct &r_job) {
   ...
   return jl1[JOB_GET_UNSTUCK].basePriority;
}
...
int assess_JobRoam(const bot_t *pBot, const job_struct &r_job) {
   ...
   return jl2[JOB_ROAM].basePriority;
}
...
int assess_JobChat(const bot_t *pBot, const job_struct &r_job) {
   ...
   return jl2[JOB_CHAT].basePriority;
}
...
int assess_JobReport(const bot_t *pBot, const job_struct &r_job) {
   ...
   return jl1[JOB_REPORT].basePriority;
}
...
int assess_JobPickUpItem(const bot_t *pBot, const job_struct &r_job) {
   // recommend the job be removed if it is invalid
   if (FNullEnt(r_job.object) || r_job.object->v.flags & FL_KILLME || r_job.object->v.effects & EF_NODRAW || r_job.f_bufferedTime < pBot->f_killed_time)
      return PRIORITY_NONE;

   // drop the job if the item is unneeded
   if (pBot->ammoStatus == AMMO_UNNEEDED && PlayerHealthPercent(pBot->pEdict) > 99 && PlayerArmorPercent(pBot->pEdict) > 99) {
   ...
   return jl1[JOB_PICKUP_ITEM].basePriority;
}
...
int assess_JobPickUpFlag(const bot_t *pBot, const job_struct &r_job) {
   ...
   return jl1[JOB_PICKUP_FLAG].basePriority;
}
...
int assess_JobPushButton(const bot_t *pBot, const job_struct &r_job) {
   // recommend the job be removed if it is invalid
   if (FNullEnt(r_job.object) || pBot->f_use_button_time + 3.0 >= pBot->f_think_time || r_job.f_bufferedTime < pBot->f_killed_time || r_job.f_bufferedTime + 15.0 < pBot->f_think_time) {
      return PRIORITY_NONE;
   }
   ...
   return jl1[JOB_PUSH_BUTTON].basePriority;
}
...
int assess_JobUseTeleport(const bot_t *pBot, const job_struct &r_job) {
   ...
   if (teleIndex != -1 && teleportGoal > -1 && pBot->telePair[teleIndex].exitWP != -1) {
      ...
      if (bestIndex != -1 && WaypointDistanceFromTo(teleportGoal, pBot->job[bestIndex].waypoint, pBot->current_team) > 500) {
         return PRIORITY_NONE; // this teleport job is no longer relevant enough
      }
   }

   return jl1[JOB_USE_TELEPORT].basePriority;
}
...
int assess_JobMaintainObject(const bot_t *pBot, const job_struct &r_job) {
   // recommend the job be removed if it is invalid
   if (pBot->pEdict->v.playerclass != TFC_CLASS_ENGINEER || pBot->bot_has_flag || FNullEnt(r_job.object) || !IsAlive(r_job.object) // NO... it can't be - it's dead!
       || r_job.object->v.flags & FL_KILLME) {
   ...
   if (pBot->m_rgAmmo[weapon_defs[TF_WEAPON_SHOTGUN].iAmmo1] < 2 && pBot->m_rgAmmo[weapon_defs[TF_WEAPON_SPANNER].iAmmo1] < 140) {
   ...
   return jl1[JOB_MAINTAIN_OBJECT].basePriority;
}
...
int assess_JobBuildSentry(const bot_t *pBot, const job_struct &r_job) {
   ...
   return jl1[JOB_BUILD_SENTRY].basePriority;
}
...
int assess_JobBuildDispenser(const bot_t *pBot, const job_struct &r_job) {
   ...
   return jl1[JOB_BUILD_DISPENSER].basePriority;
}
...
int assess_JobBuildTeleport(const bot_t *pBot, const job_struct &r_job) {
   ...
   return jl1[JOB_BUILD_TELEPORT].basePriority;
}
...
int assess_JobBuffAlly(const bot_t *pBot, const job_struct &r_job) {
   // recommend the job be removed if it is invalid
   if (FNullEnt(r_job.player) || !IsAlive(r_job.player) || r_job.f_bufferedTime < pBot->f_killed_time || r_job.f_bufferedTime + 30.0 < pBot->f_think_time)
      return PRIORITY_NONE;
   ...
   return jl1[JOB_BUFF_ALLY].basePriority;
}
...
int assess_JobEscortAlly(const bot_t *pBot, const job_struct &r_job) {
   ...
   return jl1[JOB_ESCORT_ALLY].basePriority;
}
...
int assess_JobCallMedic(const bot_t *pBot, const job_struct &r_job) {
   // recommend the job be removed if it is invalid
   if (!FNullEnt(pBot->enemy.ptr) || r_job.f_bufferedTime < pBot->f_killed_time || pBot->pEdict->v.waterlevel > WL_FEET_IN_WATER || PlayerHealthPercent(pBot->pEdict) > 99)
      return PRIORITY_NONE;

   // don't wait for a medic somewhere inappropriate
   if (pBot->current_wp > -1 && (waypoints[pBot->current_wp].flags & W_FL_LIFT || waypoints[pBot->current_wp].flags & W_FL_LADDER || waypoints[pBot->current_wp].flags & W_FL_JUMP))
      return PRIORITY_NONE;
   ...
   if (PlayerIsInfected(pBot->pEdict))
      return jl1[JOB_INFECTED_ATTACK].basePriority + 1;

   return jl1[JOB_CALL_MEDIC].basePriority;
}
...
int assess_JobGetHealth(const bot_t *pBot, const job_struct &r_job) {
   ...
   return jl1[JOB_GET_HEALTH].basePriority;
}
...
int assess_JobGetArmor(const bot_t *pBot, const job_struct &r_job) {
   ...
   return jl1[JOB_GET_ARMOR].basePriority;
}
...
int assess_JobGetAmmo(const bot_t *pBot, const job_struct &r_job) {
   ...
   return jl1[JOB_GET_AMMO].basePriority;
}
...
int assess_JobDisguise(const bot_t *pBot, const job_struct &r_job) {
   ...
   return jl1[JOB_DISGUISE].basePriority;
}
...
int assess_JobFeignAmbush(const bot_t *pBot, const job_struct &r_job) {
   ...
   return jl2[JOB_FEIGN_AMBUSH].basePriority;
}
...
int assess_JobSnipe(const bot_t *pBot, const job_struct &r_job) {
   ...
   return jl1[JOB_SNIPE].basePriority;
}
...
int assess_JobGuardWaypoint(const bot_t *pBot, const job_struct &r_job) {
   ...
   return jl2[JOB_GUARD_WAYPOINT].basePriority;
}
...
int assess_JobDefendFlag(const bot_t *pBot, const job_struct &r_job) {
   ...
   return jl1[JOB_DEFEND_FLAG].basePriority;
}
...
int assess_JobGetFlag(const bot_t *pBot, const job_struct &r_job) {
   ...
   return jl1[JOB_GET_FLAG].basePriority;
}
...
int assess_JobCaptureFlag(const bot_t *pBot, const job_struct &r_job) {
   ...
   return jl1[JOB_CAPTURE_FLAG].basePriority;
}
...
int assess_JobHarrassDefense(const bot_t *pBot, const job_struct &r_job) {
   // recommend the job be removed if it is invalid
   if (pBot->mission == ROLE_DEFENDER || pBot->bot_has_flag || pBot->pEdict->v.playerclass == TFC_CLASS_SCOUT || pBot->pEdict->v.playerclass == TFC_CLASS_CIVILIAN)
      return PRIORITY_NONE;
   ...
   return jl2[JOB_HARRASS_DEFENSE].basePriority;
}
...
int assess_JobRocketJump(const bot_t *pBot, const job_struct &r_job) {
   // recommend the job be removed if it is invalid
   if (r_job.f_bufferedTime < pBot->f_killed_time || pBot->m_rgAmmo[weapon_defs[TF_WEAPON_RPG].iAmmo1] < 4 // got enough rockets?
       || (r_job.phase == 0 && r_job.f_bufferedTime + 0.5 < pBot->f_think_time) || pBot->pEdict->v.waterlevel > WL_FEET_IN_WATER || r_job.waypoint < 0) {
...
int assess_JobDetpackWaypoint(const bot_t *pBot, const job_struct &r_job) {
   ...
   return jl2[JOB_DETPACK_WAYPOINT].basePriority;
}
...
int assess_JobPipetrap(const bot_t *pBot, const job_struct &r_job) {
   ...
   return jl2[JOB_PIPETRAP].basePriority;
}
...
int assess_JobInvestigateArea(const bot_t *pBot, const job_struct &r_job) {
   // recommend the job be removed if it is invalid
   if (!WaypointAvailable(r_job.waypoint, pBot->current_team) || r_job.f_bufferedTime < pBot->f_killed_time || r_job.f_bufferedTime + 15.0 < pBot->f_think_time)
      return PRIORITY_NONE;
   ...
   return jl1[JOB_INVESTIGATE_AREA].basePriority;
}
...
int assess_JobPursueEnemy(const bot_t *pBot, const job_struct &r_job) {
   ...
   if (r_job.f_bufferedTime < pBot->f_killed_time && pBot->f_think_time + 3.1 > pBot->f_killed_time && pBot->f_periodicAlert3 < pBot->f_think_time) {
      // know where to go?
      if (!WaypointAvailable(r_job.waypoint, pBot->current_team))
         return PRIORITY_NONE;
   ...
   return jl1[JOB_PURSUE_ENEMY].basePriority;
}
...
int assess_JobPatrolHome(const bot_t *pBot, const job_struct &r_job) {
   ...
   if (r_job.f_bufferedTime + 60.0 < pBot->f_think_time && pBot->f_periodicAlert3 < pBot->f_think_time && random_long(1, 1000) < 200)
      return PRIORITY_NONE;

   return jl2[JOB_PATROL_HOME].basePriority;
}
...
int assess_JobSpotStimulus(const bot_t *pBot, const job_struct &r_job) {
   // recommend the job be removed if it is invalid
   if (pBot->visEnemyCount > 0 || r_job.f_bufferedTime + 5.0 < pBot->f_think_time || r_job.f_bufferedTime < pBot->f_killed_time)
      return PRIORITY_NONE;

   // don't interrupt the bot if it's climbing a ladder,
   // unless it's already stuck on the ladder
   if (pBot->pEdict->v.movetype == MOVETYPE_FLY && (pBot->pEdict->v.velocity.z > 5.0 || pBot->pEdict->v.velocity.z < -5.0))
      return PRIORITY_NONE;

   return jl1[JOB_SPOT_STIMULUS].basePriority;
}
...
int assess_JobAttackBreakable(const bot_t *pBot, const job_struct &r_job) {
   ...
   return jl1[JOB_ATTACK_BREAKABLE].basePriority;
}
...
int assess_JobAttackTeleport(const bot_t *pBot, const job_struct &r_job) {
   // recommend the job be removed if it is invalid
   if (pBot->enemy.ptr != nullptr || pBot->ammoStatus < AMMO_UNNEEDED || pBot->bot_has_flag || FNullEnt(r_job.object) || !IsAlive(r_job.object) // the dead somehow live on...
       || r_job.object->v.flags & FL_KILLME || pBot->pEdict->v.playerclass == TFC_CLASS_CIVILIAN || pBot->pEdict->v.playerclass == TFC_CLASS_SNIPER || r_job.f_bufferedTime < pBot->f_killed_time)
      return PRIORITY_NONE;
   ...
   return jl2[JOB_ATTACK_TELEPORT].basePriority;
}
...
int assess_JobSeekBackup(const bot_t *pBot, const job_struct &r_job) {
   // recommend the job be removed if it is invalid
   if (r_job.f_bufferedTime < pBot->f_killed_time || (r_job.phase == 0 // abort if the job has been asleep in the buffer for too long
   ...
   return jl1[JOB_SEEK_BACKUP].basePriority;
}
...
int assess_JobAvoidEnemy(const bot_t *pBot, const job_struct &r_job) {
   ...
   return jl2[JOB_AVOID_ENEMY].basePriority;
}
...
int assess_JobAvoidAreaDamage(const bot_t *pBot, const job_struct &r_job) {
   ...
   return jl1[JOB_AVOID_AREA_DAMAGE].basePriority;
}
...
int assess_JobInfectedAttack(const bot_t *pBot, const job_struct &r_job) {
   ...
   return jl1[JOB_INFECTED_ATTACK].basePriority;
}
...
int assess_JobBinGrenade(const bot_t *pBot, const job_struct &r_job) {
   ...
   return jl1[JOB_BIN_GRENADE].basePriority;
}
...
int assess_JobDrownRecover(const bot_t *pBot, const job_struct &r_job) {
   ...
   return jl1[JOB_DROWN_RECOVER].basePriority;
}
...
int assess_JobMeleeWarrior(const bot_t *pBot, const job_struct &r_job) {
   ...
   return jl2[JOB_MELEE_WARRIOR].basePriority;
}
...
int assess_JobGraffitiArtist(const bot_t *pBot, const job_struct &r_job) {
   // recommend the job be removed if it is invalid
   if (pBot->bot_has_flag || pBot->visEnemyCount > 0 || r_job.f_bufferedTime < pBot->f_killed_time || r_job.f_bufferedTime + 25.0 < pBot->f_think_time || PlayerIsInfected(pBot->pEdict)) {
      return PRIORITY_NONE;
   }

   return jl2[JOB_GRAFFITI_ARTIST].basePriority;
}
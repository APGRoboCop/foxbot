...
int assess_JobPickUpFlag(const bot_t *pBot, const job_struct &r_job) {
   // recommend the job be removed if it is invalid
   if (pBot->bot_has_flag == TRUE || FNullEnt(r_job.object) || r_job.object->v.owner != NULL // i.e. someone is carrying it
...
int assess_JobAttackBreakable(const bot_t *pBot, const job_struct &r_job) {
   // recommend the job be removed if it is invalid
   if (FNullEnt(r_job.object) || r_job.object->v.flags & FL_KILLME || r_job.object->v.health < 0 || pBot->enemy.ptr != NULL || r_job.f_bufferedTime < pBot->f_killed_time || pBot->ammoStatus <= AMMO_LOW)
      return PRIORITY_NONE;
...
int assess_JobAttackTeleport(const bot_t *pBot, const job_struct &r_job) {
   // recommend the job be removed if it is invalid
   if (pBot->enemy.ptr != NULL || pBot->ammoStatus < AMMO_UNNEEDED || pBot->bot_has_flag || FNullEnt(r_job.object) || !IsAlive(r_job.object) // the dead somehow live on...

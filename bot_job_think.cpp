...
const jobList_struct jl[JOB_TYPE_TOTAL] = {
	...
	{ 500, "JOB_BUFF_ALLY" }, { 600, "JOB_ESCORT_ALLY" },
	...
	{ 280, "JOB_ROCKET_JUMP" }, { 720, "JOB_CONCUSSION_JUMP" }, // Reduced RJ until fix is provided [APG]RoboCop[CL]
...
void BotRunJobs(bot_t* pBot)
{
	...
	if (jf[pBot->jobType[pBot->currentJob]].jobFuncPtr != nullptr &&
...
void BotJobThink(bot_t* pBot)
{
	...
	if (pBot->pEdict->v.playerclass != TFC_CLASS_MEDIC && PlayerHealthPercent(pBot->pEdict) < pBot->trait.health &&
		...
		// if the bot saw a medic recently make it a job to call and wait for them
			...
			(newJob = InitialiseNewJob(pBot, JOB_CALL_MEDIC)) != nullptr) {
	...
	case TFC_CLASS_SPY:
		...
		if (pBot->f_spyFeignAmbushTime < pBot->f_think_time) {
			...
			if (newJob != nullptr && pBot->enemy.f_lastSeen + 3.0 < pBot->f_think_time && pBot->current_wp > -1 &&

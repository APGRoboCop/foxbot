...
const jobList_struct jl[JOB_TYPE_TOTAL] = {
    ...
    {280, "JOB_ROCKET_JUMP"},
    {720, "JOB_CONCUSSION_JUMP"}, // Reduced RJ until fix is provided [APG]RoboCop[CL]
    {390, "JOB_DETPACK_WAYPOINT"},
    {430, "JOB_PIPETRAP"},
    {590, "JOB_INVESTIGATE_AREA"},
...
void BotRunJobs(bot_t *pBot) {
   ...
   if (jf[pBot->jobType[pBot->currentJob]].jobFuncPtr != NULL && jf[pBot->jobType[pBot->currentJob]].jobFuncPtr(pBot) == JOB_TERMINATED) {
      DropJobFromBuffer(pBot, pBot->currentJob);
   }
}
...
void BotJobThink(bot_t *pBot) {
   ...
   if (pBot->pEdict->v.playerclass != TFC_CLASS_MEDIC && PlayerHealthPercent(pBot->pEdict) < pBot->trait.health && pBot->enemy.f_lastSeen + 3.0 < pBot->f_think_time) {
      ...
      else if (pBot->f_alliedMedicSeenTime + 5.0 > pBot->f_think_time && (newJob = InitialiseNewJob(pBot, JOB_CALL_MEDIC)) != NULL) {
   ...
   case TFC_CLASS_SPY:
      ...
      if (pBot->f_spyFeignAmbushTime < pBot->f_think_time) {
         ...
         if (newJob != NULL && pBot->enemy.f_lastSeen + 3.0 < pBot->f_think_time && pBot->current_wp > -1 && spawnAreaWP[pBot->current_team] > -1 && spawnAreaWP[pBot->current_team] < num_waypoints) {

...
static bot_weapon_select_t tfc_weapon_select[] = { { TF_WEAPON_KNIFE, "tf_weapon_knife", 5, 0.0, 90.0, 0.0, 0.0, 100,
	...
	{ TF_WEAPON_SUPERNAILGUN, "tf_weapon_superng", 5, 40.0, 200.0, 0.0, 0.0, 50, TRUE, 50, 1, 0, TRUE, FALSE, FALSE,
		FALSE, 0.0, 0.0 },
	{ TF_WEAPON_TRANQ, "tf_weapon_tranq", 5, 0.0, 100.0, 0.0, 0.0, 20, TRUE, 20, 1, 0, FALSE, FALSE, FALSE, FALSE,
		0.0, 0.0 },
	...
	{ TF_WEAPON_NAILGUN, "tf_weapon_ng", 5, 40.0, 300.0, 0.0, 0.0, 50, TRUE, 50, 1, 0, FALSE, FALSE, FALSE, FALSE,
		0.0, 0.0 },
	{ TF_WEAPON_RAILGUN, "tf_weapon_railgun", 5, 80.0, 200.0, 0.0, 0.0, 20, TRUE, 20, 1, 0, FALSE, FALSE, FALSE,
		FALSE, 0.0, 0.0 },
	/* terminator */
...
static edict_t* BotFindEnemy(bot_t* pBot)
{
	...
	if (mod_id == TFC_DLL) {
		...
		if (pEdict->v.playerclass == TFC_CLASS_SPY && pBot->enemy.ptr == nullptr &&
	...
	if (pBot->enemy.ptr != nullptr) {
		...
		if (FInViewCone(vecEnd, pEdict) && FVisible(vecEnd, pEdict)) {
			...
			if (pBot->lastEnemySentryGun != nullptr && pBot->enemy.ptr == pBot->lastEnemySentryGun &&
		...
		if (SubmitNewJob(pBot, JOB_REPORT, newJob) == TRUE) {
			...
			for (auto& bot : bots)
			{
				if (bot.is_used && bot.current_team == pBot->current_team &&
					bot.lastEnemySentryGun == nullptr && bot.bot_skill < 3) // skill 5's are too dumb
				{
...
bool BotFireWeapon(const Vector v_enemy, bot_t* pBot, const int weapon_choice)
{
	...
	if (pEdict->v.playerclass == TFC_CLASS_SNIPER && mod_id == TFC_DLL &&
		...
		if (pEdict->v.playerclass == TFC_CLASS_SNIPER && pBot->current_weapon.iId == TF_WEAPON_SNIPERRIFLE &&
			pBot->enemy.ptr != nullptr && mod_id == TFC_DLL && distance > 300)
			return TRUE;
	}

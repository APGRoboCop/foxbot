...
static void BotFeigningEnemyCheck(bot_t *pBot) {
   ...
   for (int i = 1; i <= gpGlobals->maxClients; i++) {
      ...
      if (pPlayer && !pPlayer->free && pPlayer != pBot->pEdict && IsAlive(pPlayer)) {
         ...
         if (VectorsNearerThan(pPlayer->v.origin, pBot->pEdict->v.origin, 400.0f) && !FInViewCone(pBot->pEdict->v.origin, pPlayer) && FVisible(pPlayer->v.origin, pBot->pEdict)) {
...
static edict_t *BotFindEnemy(bot_t *pBot) {
   ...
   if (mod_id == TFC_DLL) {
      ...
      if (pEdict->v.playerclass == TFC_CLASS_SPY && pBot->enemy.ptr == NULL && pBot->disguise_state == DISGUISE_COMPLETE && pBot->f_injured_time + 0.5f < pBot->f_think_time)
         return_null = TRUE;
   ...
   if (pBot->enemy.ptr != NULL) {
      ...
      if (FInViewCone(vecEnd, pEdict) && FVisible(vecEnd, pEdict)) {
         ...
         if (pBot->enemy.ptr != NULL) {
            ...
            if (pBot->f_snipe_time < pBot->f_think_time && (VectorsNearerThan(pBot->enemy.ptr->v.origin, pEdict->v.origin, 250.0f) || random_long(1, 1000) < 700))
               return pBot->enemy.ptr;
         } else
            return NULL;
      } else {
         // if the bot can't see it's targetted Sentry Gun fuggedaboutit
         if (pBot->lastEnemySentryGun != NULL && pBot->enemy.ptr == pBot->lastEnemySentryGun && !FNullEnt(pBot->lastEnemySentryGun))
            pBot->enemy.ptr = NULL;
   ...
   if (mod_id == TFC_DLL) {
      ...
      if (pBot->pEdict->v.playerclass == TFC_CLASS_ENGINEER && pBot->m_rgAmmo[weapon_defs[TF_WEAPON_SPANNER].iAmmo1] > 90 || pBot->pEdict->v.playerclass == TFC_CLASS_MEDIC) {
         ...
         for (i = 1; i <= gpGlobals->maxClients; i++) {
            ...
            if (pPlayer && !pPlayer->free && pPlayer != pEdict) {
               ...
               if (pBot->pEdict->v.playerclass == TFC_CLASS_MEDIC && pPlayer->v.health / pPlayer->v.max_health > 0.80 && !PlayerIsInfected(pPlayer)) // scores a point, even to selfish medics
      ...
      if (!pNewEnemy) {
         ...
         while ((pent = FIND_ENTITY_BY_CLASSNAME(pent, "building_sentrygun")) != NULL && !FNullEnt(pent)) {
            ...
            if (pBot->current_team == sentry_team || team_allies[pBot->current_team] & 1 << sentry_team) {
               if (VectorsNearerThan(pent->v.origin, pEdict->v.origin, 300.0f) && pEdict->v.playerclass != TFC_CLASS_ENGINEER && FInViewCone(vecEnd, pEdict) && FVisible(vecEnd, pEdict)) {
   ...
   if (area != -1) {
      ...
      if (SubmitNewJob(pBot, JOB_REPORT, newJob) == TRUE) {
         ...
         for (int i = 0; i < MAX_BOTS; i++) {
            if (bots[i].is_used && bots[i].current_team == pBot->current_team && bots[i].lastEnemySentryGun == NULL && bots[i].bot_skill < 3) // skill 5's are too dumb
            {
   ...
   for (int index = 0; index < num_waypoints; index++) {
      ...
      if (VectorsNearerThan(waypoints[index].origin, r_vecOrigin, 500.0)) {
         ...
         if (tr.flFraction >= 1.0) {
            UTIL_TraceLine(pBot->pEdict->v.origin + pBot->pEdict->v.view_ofs, waypoints[index].origin, ignore_monsters, NULL, &tr);
...
void BotShootAtEnemy(bot_t *pBot) {
   ...
   if (bot_use_grenades && pBot->pEdict->v.playerclass == TFC_CLASS_SPY && pBot->enemy.ptr == pBot->lastEnemySentryGun && pBot->disguise_state == DISGUISE_COMPLETE && !FNullEnt(pBot->lastEnemySentryGun) &&
       pBot->grenades[PrimaryGrenade] > 0 && f_distance < 1200.0f) {
...
bool BotFireWeapon(const Vector v_enemy, bot_t *pBot, const int weapon_choice) {
   ...
   if (pEdict->v.playerclass == TFC_CLASS_SNIPER && mod_id == TFC_DLL && (pEdict->v.waterlevel >= WL_WAIST_IN_WATER || pEdict->v.movetype == MOVETYPE_FLY) &&
      ...
      if (pEdict->v.playerclass == TFC_CLASS_SNIPER && pBot->current_weapon.iId == TF_WEAPON_SNIPERRIFLE && pBot->enemy.ptr != NULL && mod_id == TFC_DLL && distance > 300)
         return TRUE;
   }

		...
		else // everything else...
		{
			...
			if (BotCanSeeOrigin(pBot, vecEnd)) {
				...
				// check if entity is a dispenser  // Fixed for Foxbot 0.79? [APG]RoboCop[CL]
					...
					if (pBot->ammoStatus == AMMO_UNNEEDED && pBot->bot_has_flag == false) {
...
static void BotAmmoCheck(bot_t* pBot) {
	...
	case TFC_CLASS_SCOUT:
		if (pBot->m_rgAmmo[weapon_defs[TF_WEAPON_SHOTGUN].iAmmo1] >= PC_SCOUT_MAXAMMO_SHOT / 2 && pBot->m_rgAmmo[weapon_defs[TF_WEAPON_NAILGUN].iAmmo1] >= PC_SCOUT_MAXAMMO_NAIL / 2)
			pBot->ammoStatus = AMMO_UNNEEDED;
		else if (pBot->ammoStatus != AMMO_LOW)
			pBot->ammoStatus = AMMO_WANTED;
		break;
	case TFC_CLASS_SNIPER:
		if (pBot->m_rgAmmo[weapon_defs[TF_WEAPON_SHOTGUN].iAmmo1] >= PC_SNIPER_MAXAMMO_SHOT / 2 && pBot->m_rgAmmo[weapon_defs[TF_WEAPON_NAILGUN].iAmmo1] >= PC_SNIPER_MAXAMMO_NAIL / 2)
			pBot->ammoStatus = AMMO_UNNEEDED;
		else if (pBot->ammoStatus != AMMO_LOW)
			pBot->ammoStatus = AMMO_WANTED;
		break;
	case TFC_CLASS_SOLDIER:
		if (pBot->m_rgAmmo[weapon_defs[TF_WEAPON_SHOTGUN].iAmmo1] >= PC_SOLDIER_MAXAMMO_SHOT / 2 && pBot->m_rgAmmo[weapon_defs[TF_WEAPON_RPG].iAmmo1] >= PC_SOLDIER_MAXAMMO_ROCKET / 2)
			pBot->ammoStatus = AMMO_UNNEEDED;
		else if (pBot->ammoStatus != AMMO_LOW)
			pBot->ammoStatus = AMMO_WANTED;
		break;
	case TFC_CLASS_DEMOMAN:
		if (pBot->m_rgAmmo[weapon_defs[TF_WEAPON_SHOTGUN].iAmmo1] >= PC_DEMOMAN_MAXAMMO_SHOT / 2 && pBot->m_rgAmmo[weapon_defs[TF_WEAPON_RPG].iAmmo1] >= PC_DEMOMAN_MAXAMMO_ROCKET / 2)
			pBot->ammoStatus = AMMO_UNNEEDED;
		else if (pBot->ammoStatus != AMMO_LOW)
			pBot->ammoStatus = AMMO_WANTED;
		break;
	case TFC_CLASS_MEDIC:
		if (pBot->m_rgAmmo[weapon_defs[TF_WEAPON_SHOTGUN].iAmmo1] >= PC_MEDIC_MAXAMMO_SHOT / 2 && pBot->m_rgAmmo[weapon_defs[TF_WEAPON_NAILGUN].iAmmo1] >= PC_MEDIC_MAXAMMO_NAIL / 2)
			pBot->ammoStatus = AMMO_UNNEEDED;
		else if (pBot->ammoStatus != AMMO_LOW)
			pBot->ammoStatus = AMMO_WANTED;
		break;
	case TFC_CLASS_HWGUY:
		...
		break;
	case TFC_CLASS_PYRO:
		if (pBot->m_rgAmmo[weapon_defs[TF_WEAPON_SHOTGUN].iAmmo1] >= PC_PYRO_MAXAMMO_SHOT / 2 && pBot->m_rgAmmo[weapon_defs[TF_WEAPON_FLAMETHROWER].iAmmo1] >= PC_PYRO_MAXAMMO_CELL / 2 &&
		...
		break;
	case TFC_CLASS_SPY:
		if (pBot->m_rgAmmo[weapon_defs[TF_WEAPON_SHOTGUN].iAmmo1] >= PC_SPY_MAXAMMO_SHOT / 2 && pBot->m_rgAmmo[weapon_defs[TF_WEAPON_NAILGUN].iAmmo1] >= PC_SPY_MAXAMMO_NAIL / 2)
			pBot->ammoStatus = AMMO_UNNEEDED;
		else if (pBot->ammoStatus != AMMO_LOW)
			pBot->ammoStatus = AMMO_WANTED;
		break;
	case TFC_CLASS_ENGINEER:
		if (pBot->m_rgAmmo[weapon_defs[TF_WEAPON_SPANNER].iAmmo1] == PC_ENGINEER_MAXAMMO_CELL && pBot->m_rgAmmo[weapon_defs[TF_WEAPON_SHOTGUN].iAmmo1] >= PC_ENGINEER_MAXAMMO_SHOT - 10 &&
			pBot->m_rgAmmo[weapon_defs[TF_WEAPON_NAILGUN].iAmmo1] >= PC_ENGINEER_MAXAMMO_NAIL / 2 && pBot->m_rgAmmo[weapon_defs[TF_WEAPON_RPG].iAmmo1] == PC_ENGINEER_MAXAMMO_ROCKET)
			pBot->ammoStatus = AMMO_UNNEEDED;
		else if (pBot->ammoStatus != AMMO_LOW)
			pBot->ammoStatus = AMMO_WANTED;
		break;
...
edict_t* BotContactThink(bot_t* pBot) {
	...
	if (playerToAvoid != nullptr) {
		...
		if (pBot->current_wp != pBot->goto_wp && pBot->current_wp != -1 && pBot->pEdict->v.flags & FL_ONGROUND && VectorsNearerThan(playerToAvoid->v.origin, waypoints[pBot->current_wp].origin, 40.0) &&
			VectorsNearerThan(pBot->pEdict->v.origin, waypoints[pBot->current_wp].origin, 100.1) && playerToAvoid->v.velocity.Length2D() < pBot->pEdict->v.velocity.Length2D()) {
			int nextWP;
			...
			if (nextWP != -1 && !(waypoints[pBot->current_wp].flags & W_FL_LIFT) && !(waypoints[pBot->current_wp].flags & W_FL_JUMP) && BotCanSeeOrigin(pBot, waypoints[pBot->current_wp].origin))
...
void BotSprayLogo(edict_t* pEntity, const bool sprayDownwards) {
   ...
   TraceResult pTrace;
	...
	if (pTrace.pHit && pTrace.flFraction < 1.0f) {
		...
		EMIT_SOUND_DYN2(pEntity, CHAN_VOICE, "player/sprayer.wav", 1.0f, ATTN_NORM, 0, 100);
	}
}
...
static void BotAttackerCheck(bot_t* pBot) {
	...
	if (mysteryPlayer != nullptr) {
		...
		if (newJob != nullptr) {
			newJob->waypoint = WaypointFindNearest_V(mysteryPlayer->v.origin, 500.0f, pBot->current_team);
...
static void BotRoleCheck(bot_t* pBot) {
   ...
   for (short team = 0; team < MAX_TEAMS; team++) {
		...
		if (needed_mission == ROLE_DEFENDER && WaypointTypeExists(W_FL_TFC_DETPACK_CLEAR | W_FL_TFC_DETPACK_SEAL, team) == true)
			ignoreDemomen = true;
...
static void BotComms(bot_t* pBot) {
	...
	if (pBot->newmsg && pBot->f_periodicAlert1 < pBot->f_think_time) {
		...
		while (k) {
			...
			if (strcasecmp("changeclass", cmd) == 0 || strcasecmp("changeclassnow", cmd) == 0) {
				...
				if (std::strlen(pBot->message) >= 2) {
					theClass = pBot->message[std::strlen(pBot->message) - 2];
				}
				if (theClass != '\0' && std::strchr("123456789", theClass)) {
					std::string theClassStr(1, theClass);
					const int iClass = std::atoi(theClassStr.c_str());
			...
			if (strcasecmp("xmas", cmd) == 0 && bot_xmas) {
				...
				if (random_long(0, 100) <= 50) {
					EMIT_SOUND_DYN2(pBot->pEdict, CHAN_VOICE, "misc/b2.wav", 1.0f, ATTN_NORM, 0, 100);
				}
				else if (random_long(0, 100) <= 50) {
					EMIT_SOUND_DYN2(pBot->pEdict, CHAN_VOICE, "misc/party2.wav", 1.0f, ATTN_NORM, 0, 100);
				}
				else if (random_long(0, 100) <= 50) {
					EMIT_SOUND_DYN2(pBot->pEdict, CHAN_VOICE, "misc/party1.wav", 1.0f, ATTN_NORM, 0, 100);
				}
			}
		}
	...
	if (pBot->killer_edict != nullptr && random_long(1, 1000) < static_cast<long>(bot_chat)) {
		if (pBot->killer_edict->v.frags > pEdict->v.frags) {
			...
			chat.pickRandomChatString(newJob->message, MAX_CHAT_LENGTH, CHAT_TYPE_KILLED_HI, STRING(pBot->killer_edict->v.netname));
		}
	...
	if (pBot->killed_edict != nullptr && random_long(1, 1000) < static_cast<long>(bot_chat)) {
		if (pBot->killed_edict->v.frags > pEdict->v.frags) {
			...
			chat.pickRandomChatString(newJob->message, MAX_CHAT_LENGTH, CHAT_TYPE_KILL_HI, STRING(pBot->killed_edict->v.netname));
		}
	...
	if (bot_bhop > 0 && pBot->f_bunnyHopTime < pBot->f_think_time
		...
		&& !BufferContainsJobType(pBot, JOB_CONCUSSION_JUMP))
	   {
		...
		if (fabsf(yawError) > 45.0f) {
			...
			// If the bot is hopping but not gaining momentum, stop hopping - [APG]RoboCop[CL]
			const float horizontalSpeed = pBot->pEdict->v.velocity.Length2D();

			if (horizontalSpeed < pBot->f_max_speed * 0.75f) {
				// not moving fast enough - hopping is counterproductive
				pBot->f_bunnyHopTime = pBot->f_think_time + random_float(1.5f, 3.0f);
			}
			else {
...
static void BotCombatThink(bot_t* pBot) {
	...
	if ((ThreatLevel <= 25 || pBot->enemy.seenWithFlag) && !pBot->bot_has_flag && pBot->f_periodicAlert1 < pBot->f_think_time) {
		...
		if (pBot->pEdict->v.playerclass == TFC_CLASS_SNIPER && (pBot->trait.aggression < 50 || waypoints[pBot->current_wp].flags & W_FL_SNIPER))
			tooBusySniping = true;
	...
	if (ThreatLevel >= 66 && (pBot->pEdict->v.playerclass == TFC_CLASS_CIVILIAN || !pBot->bot_has_flag)) // the civilian has an invisible flag in hunted
	{
	...
	if (pBot->pEdict->v.playerclass == TFC_CLASS_DEMOMAN && (pBot->current_weapon.iId == TF_WEAPON_GL || pBot->current_weapon.iId == TF_WEAPON_PL) && pBot->current_weapon.iClip == 0) {
	   ...
	   else if (spectate_debug == 3) // show the bots personality traits
      {
		...
		if (pBot->f_periodicAlertFifth < pBot->f_think_time) {
			if (pBot->goto_wp > -1 && pBot->goto_wp < num_waypoints)
				WaypointDrawBeam(INDEXENT(1), pBot->pEdict->v.origin, waypoints[pBot->goto_wp].origin, 10, 2, 50, 50, 250, 200, 10);

			// if the bot is taking a side route, show it
			if (pBot->branch_waypoint != -1)
				WaypointDrawBeam(INDEXENT(1), pBot->pEdict->v.origin + pBot->pEdict->v.view_ofs, waypoints[pBot->branch_waypoint].origin + Vector(0, 0, 35.0), 10, 2, 250, 250, 50, 200, 10);
		}

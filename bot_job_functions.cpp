...
int JobPickUpItem(bot_t *pBot) {
   ...
   if (job_ptr->phase == 1) {
      ...
      if (pBot->pEdict->v.origin.z > job_ptr->object->v.origin.z - 37.0 && pBot->pEdict->v.origin.z < job_ptr->object->v.origin.z + 40.0) {
...
int JobPickUpFlag(bot_t *pBot) {
   ...
   if (job_ptr->phase == 1) {
      ...
      if (pBot->pEdict->v.origin.z > job_ptr->object->v.origin.z - 37.0 && pBot->pEdict->v.origin.z < job_ptr->object->v.origin.z + 60.0) {
         if ((job_ptr->object->v.origin - pBot->pEdict->v.origin).Length2D() < 25.0)
            job_ptr->phase = 2;
      }
...
int JobUseTeleport(bot_t *pBot) {
   ...
   if (job_ptr->phase == 1) {
      ...
      if (pBot->pEdict->v.origin.z > TeleportCenter.z - 10.0 && pBot->pEdict->v.origin.z < TeleportCenter.z + 90.0 && distance2D < 20.0) {
...
int JobEscortAlly(bot_t *pBot) {
   ...
   constexpr float maxEscortRange = 500.0;
...
int JobDisguise(bot_t *pBot) {
   ...
   if (pBot->disguise_state == DISGUISE_NONE) {
      static constexpr int disguiseList[] = {
          2, 3, 4, 7, 8,
      };
...
int JobSnipe(bot_t *pBot) {
   ...
   if (job_ptr->phase == 0) {
      ...
      if (pBot->pEdict->v.flags & FL_ONGROUND && (pBot->current_wp == job_ptr->waypoint || nextWP == job_ptr->waypoint) && zDiff > -45.1 && zDiff < 15.0 // walkable step height(hopefully)
         ...
         constexpr float baseChargeTime[5] = {2.0, 1.0, 0.0, -5.0, -5.0};
...
int JobSpotStimulus(bot_t *pBot) {
   ...
   if (job_ptr->phase == 0) {
      ...
      if (pBot->current_wp != job_ptr->waypoint && job_ptr->waypoint > -1 && job_ptr->waypoint < num_waypoints) {
         ...
         {
            if (pBot->trait.aggression > 50 || pBot->pEdict->v.waterlevel == WL_HEAD_IN_WATER || pBot->f_injured_time + 2.0 > pBot->f_think_time || random_long(0, 1000) > 300)
               moveNlook = true;
         }
      }

...
int JobReport(bot_t* pBot) {
	...
	if (job_ptr->phase == 1 && job_ptr->phase_timer < pBot->f_think_time) {
		...
		BlacklistJob(pBot, JOB_REPORT, 5.0f); // don't report the same thing too often [APG]RoboCop[CL]
		return JOB_TERMINATED; // job done
	}
	...
	if (job_ptr->phase == 0) {
		if (pBot->current_wp == job_ptr->waypoint) {
			...
			if (distance > 30.0f) {
				...
				if (BotEntityAtPoint("building_dispenser", waypoints[pBot->current_wp].origin, 300.0f) != nullptr) {
		...
		if (job_ptr->phase_timer < pBot->f_think_time) {
			...
			while ((pent = FIND_ENTITY_BY_CLASSNAME(pent, "building_dispenser")) != nullptr && !FNullEnt(pent)) {
				...
				if (VectorsNearerThan(job_ptr->origin, pent->v.origin, 50.0)) {
					...
					pBot->f_dispenserDetTime = 0.0f; // don't blow it up yet!
					return JOB_TERMINATED;          // job done!
				}
			}
...
int JobGetHealth(bot_t* pBot) {
	...
	if (job_ptr->phase == 0) {
		...
		if (pBot->f_periodicAlert1 < pBot->f_think_time && WaypointDistanceFromTo(pBot->current_wp, job_ptr->waypoint, pBot->current_team) < 800) {
	...
	if (job_ptr->phase == 3) {
		...
		if (pBot->f_view_change_time <= pBot->f_think_time) {
			...
			if (aim_index != -1) {
				...
				pBot->pEdict->v.ideal_yaw = aim_angles.y + static_cast<float>(random_long(0, 40) - 20);
				pBot->pEdict->v.idealpitch = 0 + static_cast<float>(random_long(0, 20) - 10);
			}
		}
	}
	...
	if (pBot->current_wp == job_ptr->waypoint && VectorsNearerThan(pBot->pEdict->v.origin, waypoints[job_ptr->waypoint].origin, 50.0)) {
		...
		if (pBot->f_view_change_time <= pBot->f_think_time) {
			...
			if (aim_index != -1) {
				...
				const Vector aim_angles = UTIL_VecToAngles(v_aim);
				pBot->pEdict->v.ideal_yaw = aim_angles.y + static_cast<float>(random_long(0, 60) - 30);
				pBot->pEdict->v.idealpitch = 0 + static_cast<float>(random_long(0, 20) - 10);
			}
		}
	}
...
int JobGetFlag(bot_t* pBot) {
	...
	if (job_ptr->phase == 1) {
		...
		if (!VectorsNearerThan(waypoints[job_ptr->waypoint].origin, pBot->pEdict->v.origin, 20.0)) {
			...
			// randomly crouch
			if (pBot->trait.aggression < 30 && pBot->f_periodicAlert1 < pBot->f_think_time && pBot->visEnemyCount < 1 && random_long(1, 1000) < 200)
...
int JobConcussionJump(bot_t* pBot) {
	...
	if (job_ptr->phase == 2) {
		...
		if (pBot->pEdict->v.velocity.z < -1.0f) {
	...
	if (job_ptr->phase == 3) {
		...
		if (pBot->f_view_change_time <= pBot->f_think_time) {
			...
			if (aim_index != -1) {
				const Vector v_aim = waypoints[aim_index].origin - waypoints[pBot->current_wp].origin;
				const Vector aim_angles = UTIL_VecToAngles(v_aim);
				pBot->pEdict->v.ideal_yaw = aim_angles.y + static_cast<float>(random_long(0, 90) - 45);
				pBot->pEdict->v.idealpitch = 0 + static_cast<float>(random_long(0, 20) - 10);
			}
		}
	}
...
int JobSpotStimulus(bot_t* pBot) {
	...
	if (job_ptr->phase == 0) {
		...
		if (pBot->current_wp != job_ptr->waypoint && job_ptr->waypoint > -1 && job_ptr->waypoint < num_waypoints) {
			...
			{
				if (pBot->trait.aggression > 50 || pBot->pEdict->v.waterlevel == WL_HEAD_IN_WATER || pBot->f_injured_time + 2.0f > pBot->f_think_time || random_long(0, 1000) > 300)
					moveNlook = true;
			}
		}
		...
		if (moveNlook == true) {
			job_ptr->phase = 2;
			job_ptr->phase_timer = pBot->f_think_time + 0.5f + static_cast<float>(pBot->bot_skill) * 0.5f + random_float(0.5f, 1.5f);
		}
		else {
			job_ptr->phase = 1;

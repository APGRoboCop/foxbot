...
//extern int gmsgSayText = 0;
//extern int gmsgTextMsg = 0;
int gmsgSetFOV = 0;
//extern int gmsgShowMenu = 0;
...
{
	Vector vec(RANDOM_FLOAT(-100.0f, 100.0f), RANDOM_FLOAT(-100.0f, 100.0f), RANDOM_FLOAT(200.0f, 300.0f));
...
int TrainSpeed(int iSpeed, int iMax)
{
	...
	float fMax = static_cast<float>(iMax);
	float fSpeed = iSpeed;
...
// TraceAttack
//=========================================================
void CBasePlayer :: TraceAttack( entvars_t *pevAttacker,
	float flDamage, Vector vecDir, TraceResult *ptr, int bitsDamageType)
{
	if( pev->takedamage )
	{
...
int CBasePlayer :: TakeDamage( entvars_t *pevInflictor,
	entvars_t *pevAttacker, float flDamage, int bitsDamageType )
{
	...
	fTookDamage = CBaseMonster::TakeDamage(pevInflictor, pevAttacker, static_cast<int>(flDamage), bitsDamageType);
...
void CBasePlayer::Killed( entvars_t *pevAttacker, int iGib )
{
	...
	if(pev->velocity.z < 10)
		pev->velocity.z += RANDOM_FLOAT(0, 300.0f);
	...
	if( ( pev->health < -40 && iGib != GIB_NEVER )
		|| iGib == GIB_ALWAYS )
	{
		pev->solid			= SOLID_NOT;
...
void CBasePlayer::SetAnimation( PLAYER_ANIM playerAnim )
{
	...
	case PLAYER_WALK:
		if( !FBitSet( pev->flags, FL_ONGROUND )
			&& (m_Activity == ACT_HOP || m_Activity == ACT_LEAP) )// Still jumping
		{
			m_IdealActivity = m_Activity;
		}
...
void CBasePlayer::WaterMove()
{
	...
	{	// fully under water
		...
		if(pev->air_finished < gpGlobals->time)		// drown!
		{
			if(pev->pain_finished < gpGlobals->time)
			{
				...
				if(pev->dmg > 5)
					pev->dmg = 5;
				TakeDamage(VARS(eoNullEntity), VARS(eoNullEntity),
					pev->dmg, DMG_DROWN);
	...
	if(!RANDOM_LONG(0,0x1f) && RANDOM_LONG(0,AIRTIME-1) >= air)
	{
		switch (RANDOM_LONG(0,3))
			{
			case 0:
				EMIT_SOUND(ENT(pev), CHAN_BODY, "player/pl_swim1.wav",
					0.8, ATTN_NORM);
				break;
			case 1:
				EMIT_SOUND(ENT(pev), CHAN_BODY, "player/pl_swim2.wav",
					0.8, ATTN_NORM);
				break;
			case 2:
				EMIT_SOUND(ENT(pev), CHAN_BODY, "player/pl_swim3.wav",
					0.8, ATTN_NORM);
				break;
			case 3:
				EMIT_SOUND(ENT(pev), CHAN_BODY, "player/pl_swim4.wav",
					0.8, ATTN_NORM);
				break;
		}
	}
	...
	if(pev->watertype == CONTENT_LAVA)		// do damage
	{
		if(pev->dmgtime < gpGlobals->time)
			TakeDamage(VARS(eoNullEntity), VARS(eoNullEntity),
				10 * pev->waterlevel, DMG_BURN);
	}
	else if(pev->watertype == CONTENT_SLIME)		// do damage
	{
		pev->dmgtime = gpGlobals->time + 1;
		TakeDamage(VARS(eoNullEntity), VARS(eoNullEntity),
			4 * pev->waterlevel, DMG_ACID);
	}
...
{
	// direction we're tracing a line to find a wall when walljumping
	Vector		vecWallCheckDir;
...
// This is a glorious hack to find free space when you've crouched into
// some solid space.
// Our crouching collisions do not work correctly for some reason and
// this is easier than fixing the problem :(
void FixPlayerCrouchStuck( edict_t *pPlayer )
{
	...
	for( int i = 0; i < 18; i++ )
	{
		UTIL_TraceHull( pPlayer->v.origin, pPlayer->v.origin,
			dont_ignore_monsters, head_hull, pPlayer, &trace );
...
void CBasePlayer::ImpulseCommands( )
{
	...
	int iImpulse = static_cast<int>(pev->impulse);
	switch (iImpulse)
	{
	case 99:
		{
...
void CBasePlayer :: UpdateClientData( void )
{
	...
	if(pev->armorvalue != m_iClientBattery)
	{
		...
		MESSAGE_BEGIN( MSG_ONE, gmsgBattery, NULL, pev );
			WRITE_SHORT( static_cast<int>(pev->armorvalue));
		MESSAGE_END();
	}
...
Vector CBasePlayer :: AutoaimDeflection( Vector &vecSrc, float flDist,
	float flDelta  )
{
...
char *CDeadHEV::m_szPoses[]
	= { "deadback", "deadsitting", "deadstomach", "deadtable" };
